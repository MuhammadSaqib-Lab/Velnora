...
- **The cookie** (`velnora_admin_session`) is `httpOnly` (never readable from page JS), `secure` in production, and `SameSite=Lax` in every environment — this only stays correct because the frontend proxies `/api/*` to the backend (root `vercel.json` in production, `vite.config.ts`'s dev-server proxy locally) rather than calling it cross-origin directly, so every request looks same-site to the browser. See SECURITY.md's "Admin Dashboard security" section for the two broken iterations this went through live (`Lax` while calling Render directly, then `None` which hit third-party-cookie blocking) before landing on the proxy as the actual fix.
