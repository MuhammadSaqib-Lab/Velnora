...
- **The cookie** (`velnora_admin_session`) is `httpOnly` (never readable from page JS), `secure` in production, and `SameSite=Lax` — see SECURITY.md's "Admin Dashboard security" section for why Lax is both correct and the actual CSRF defense for this specific frontend/backend architecture.
