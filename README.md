...
- `vercel.json`'s CSP `connect-src` now points at the real deployed backend (`https://velnora-41qv.onrender.com`, set once Render was actually live) instead of the old `https://api.velnora.com` placeholder — if the backend ever moves to a different host/domain, update this exact origin here too (no wildcard), or the frontend's API calls get silently blocked by the browser's own CSP
