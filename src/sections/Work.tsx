...
// Generated by scripts/optimize-images.mjs alongside each full-size original
// in public/work/ — keep this in sync with that script's `widths` array if
// it ever changes.
const RESPONSIVE_WIDTHS = [480, 640, 960, 1280]

const WORK_IMAGE_SIZES = {
  // The first card spans 2 of 3 grid columns at lg, 1 of 2 at md.
  featured: '(min-width: 1024px) 66vw, (min-width: 768px) 50vw, 100vw',
  default: '(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw',
}

function withResizedSuffix(imagePath: string, width: number) {
  return imagePath.replace(/\.jpg$/, `-${width}w.jpg`)
}

function buildSrcSet(imagePath: string, ext: 'avif' | 'webp' | 'jpg') {
  return RESPONSIVE_WIDTHS.map(
    (w) => `${imagePath.replace(/\.jpg$/, `-${w}w.${ext}`)} ${w}w`,
  ).join(', ')
}
                ...
                <div className="relative overflow-hidden">
                  ...
                  {project.isConcept ? (
                    ...
                    <picture>
                      <source
                        type="image/avif"
                        srcSet={buildSrcSet(project.image!, 'avif')}
                        sizes={i === 0 ? WORK_IMAGE_SIZES.featured : WORK_IMAGE_SIZES.default}
                      />
                      <source
                        type="image/webp"
                        srcSet={buildSrcSet(project.image!, 'webp')}
                        sizes={i === 0 ? WORK_IMAGE_SIZES.featured : WORK_IMAGE_SIZES.default}
                      />
                      <img
                        src={withResizedSuffix(project.image!, 1280)}
                        srcSet={buildSrcSet(project.image!, 'jpg')}
                        sizes={i === 0 ? WORK_IMAGE_SIZES.featured : WORK_IMAGE_SIZES.default}
